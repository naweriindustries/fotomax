...
<thead>
 ...
 <tbody id="myTable">
						...
						<?php $getinstantaccessdata=mysqli_query($benzima,"SELECT * FROM allaccounts WHERE acagestatus='new'  ORDER BY indexkey DESC ");
						     while($instantdata=mysqli_fetch_array($getinstantaccessdata))

				{

					
					$altitle=htmlentities($instantdata['acids']);
					$instantindex=htmlentities($instantdata['accid']);
					$statusinstant=htmlentities($instantdata['facerecog']);
					$dateofmessagenew=htmlentities($instantdata['acdatesdata']);
					$indexkey=htmlentities($instantdata['indexkey']);
...
<td>
	<div class="profile-userpic">
		<img src="../acproofs/<?php echo$statusinstant;?>"  class="img-responsive">
	</div>
</td>
...
<td><?php echo$instantindex;?></td>
	...
	<td><a href="user_details.php?userindex=<?php echo$indexkey;?>" target="_blank"><button class="btn btn-success"><i class="fa fa-eye"></i></button></a></td>
